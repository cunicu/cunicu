...
var _ = Context("duration", func() {
	DescribeTable("test", func(dur time.Duration, output string) {
		Expect(util.PrettyDuration(dur)).To(Equal(output))
	},
...
var _ = Specify("ago", func() {
	...
	Expect(util.Ago(now)).To(Equal("Now"))
	Expect(util.Ago(now.Add(-time.Hour))).To(Equal("1 hour ago"))
	Expect(util.Ago(now.Add(-time.Hour - 10*time.Minute))).To(Equal("1 hour, 10 minutes ago"))
})

var _ = Context("bytes", func() {
	DescribeTable("test", func(bytes int, output string) {
		Expect(util.PrettyBytes(int64(bytes))).To(Equal(output))
	},
...
var _ = Context("every", func() {
	DescribeTable("test", func(dur time.Duration, output string) {
		Expect(util.Every(dur)).To(Equal(output))
	},
